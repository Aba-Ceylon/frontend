...
export default function PackageCard({ pkg }: { pkg: PackageItem }) {
  ...
  const packageLabel = pkg.packageId
    ? t("packages.packageLabel", { id: pkg.packageId })
    : t("packages.curatedPackage");
    ...
    >
      ...
      <div className="p-3 sm:p-4 md:p-5">
        ...
        <p className="text-xs sm:text-sm text-neutral-600 mb-2">
          {t("packages.distanceKm", { km: pkg.km })}
        </p>
        ...
        <div className="flex gap-2 sm:gap-3 flex-col xl:flex-row">
          <PackageRequestButton
            pkg={pkg}
            label={t("packages.requestPackage")}
            requestedLabel={t("packages.requested")}
