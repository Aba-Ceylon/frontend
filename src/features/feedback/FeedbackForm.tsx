...
export default function FeedbackForm() {
  ...
  const ratingLabels = [
    ...
    t("feedback.ratingLabels.5"),
  ];

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (rating === 0) { dispatch({ type: "SET_STATUS", status: "idle", errorMsg: t("feedback.errors.selectRating") }); return; }
    if (message.trim().length < 20) { dispatch({ type: "SET_STATUS", status: "idle", errorMsg: t("feedback.errors.minimumMessage") }); return; }
    ...
    if (result.success) {
      ...
      dispatch({ type: "SET_STATUS", status: "error", errorMsg: result.error ?? t("feedback.errors.generic") });
    }
  }
    ...
    >
      ...
      {status === "success" ? (
        <div className="text-center py-8">
          ...
          <p className="font-cinzel text-amber-400 text-lg mb-2">{t("feedback.thankYou")}</p>
          <p className="text-slate-400 text-sm">{t("feedback.success")}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block font-cinzel text-sm text-slate-300 mb-3">{t("feedback.rateExperience")}</label>
            ...
            {rating > 0 && <p className="text-xs text-amber-400/70 mt-1 font-cinzel">{ratingLabels[rating]}</p>}
          </div>
