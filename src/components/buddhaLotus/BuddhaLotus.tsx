...
const PETAL_PATH = "M30 100 C20 100, 4 80, 2 58 C0 38, 10 16, 30 2 C50 16, 60 38, 58 58 C56 80, 40 100, 30 100 Z";
const INNER_PETAL_PATH = "M30 100 C22 100, 8 82, 6 62 C4 44, 14 22, 30 8 C46 22, 56 44, 54 62 C52 82, 38 100, 30 100 Z";

const LAYER_DEFS = [
  { count: 8,  pw: 22, ph: 52,  path: INNER_PETAL_PATH, offset: 0,  zIndex: 5, from: "#C8960C", to: "#F0D060" },
  { count: 10, pw: 28, ph: 68,  path: PETAL_PATH,        offset: 18, zIndex: 4, from: "#D4AF37", to: "#F5E090" },
  { count: 12, pw: 34, ph: 82,  path: PETAL_PATH,        offset: 8,  zIndex: 3, from: "#EDD98A", to: "#FFF8F0" },
  { count: 14, pw: 38, ph: 94,  path: PETAL_PATH,        offset: 4,  zIndex: 2, from: "#F8EED0", to: "#FFFFFF" },
  { count: 16, pw: 40, ph: 102, path: PETAL_PATH,        offset: 11, zIndex: 1, from: "#F5F0E8", to: "#FFFFFF" },
];
...
export default function BuddhaLotus() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const allPetals = lotusRef.current
        ? Array.from(lotusRef.current.querySelectorAll<SVGElement>(".lotus-petal-svg"))
        : [];
      ...
      // 0–100%: background darkens, glass blur intensifies
      ...
      tl.to(centerRef.current, { scale: 1, opacity: 1, duration: 0.06, ease: "back.out(2)" }, 0.02);

      // 0–55%: lotus grows small → large
      tl.to(lotusRef.current, { scale: 1.45, duration: 0.55, ease: "power2.out", transformOrigin: "50% 50%" }, 0);

      // 2–50%: petals bloom layer by layer
      layerPetals.forEach((petals, li) => {
        const layerStart = 0.02 + li * 0.11;
        petals.forEach((petal, pi) => {
          tl.to(petal, { scale: 1, opacity: 1, duration: 0.06, ease: "back.out(1.6)" }, layerStart + pi * (0.08 / petals.length));
        });
      });

      // 55–70%: lotus settles, side panels slide in
      tl.to(lotusRef.current, { scale: 1.15, duration: 0.15, ease: "power2.inOut", transformOrigin: "50% 50%" }, 0.55);
      tl.to(leftPanelRef.current, { opacity: 1, x: 0, duration: 0.12 }, 0.52);
      tl.to(rightPanelRef.current, { opacity: 1, x: 0, duration: 0.12 }, 0.52);

      // 45–65%: mandala + rings, glow shifts gold → rose
      tl.to(mandalaRef.current, { scale: 1, opacity: 1, duration: 0.15, ease: "power2.out" }, 0.45);
      ...
      // 48–68%: color shift outer layers → white
      layerPetals.slice(2).forEach((petals, li) => {
        ...
        petals.forEach((petal) => {
          ...
          if (stop) tl.to(stop, { attr: { stopColor: colorTo }, duration: 0.16 }, shiftStart);
        });
      });
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(160deg, #f9f5ee 0%, #f5f0e8 40%, #fdf6f0 100%)" }}
    >
      {/* Dark overlay */}
      <div ref={bgOverlayRef} className="absolute inset-0 pointer-events-none z-0" style={{ backgroundColor: "#1A2238" }} />

      {/* Frosted glass */}
      <div
        ref={glassRef}
        className="absolute inset-0 pointer-events-none z-[1]"
        style={{ backdropFilter: "blur(0px)", WebkitBackdropFilter: "blur(0px)", background: "rgba(26, 34, 56, 0.08)" }}
      />
      ...
      {/* Corner ornaments */}
      {["top-6 left-6", "top-6 right-6 scale-x-[-1]", "bottom-6 left-6 scale-y-[-1]", "bottom-6 right-6 scale-[-1]"].map((pos, i) => (
        <div key={i} className={`absolute ${pos} pointer-events-none opacity-40 z-[3]`}>
          <svg width="60" height="60" viewBox="0 0 60 60" fill="none">
            <path d="M4 4 L4 20 M4 4 L20 4" stroke="#B8860B" strokeWidth="1.5" strokeLinecap="round" />
            <circle cx="4" cy="4" r="2" fill="#B8860B" />
            <path d="M14 14 L14 24 M14 14 L24 14" stroke="#B8860B" strokeWidth="0.8" strokeLinecap="round" opacity="0.6" />
          </svg>
        </div>
      ))}

      {/* Top badge */}
      <div ref={topBadgeRef} className="absolute top-8 left-0 right-0 flex flex-col items-center pointer-events-none z-[3]">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-cinzel text-amber-600/50 text-xs">✦</span>
          <div className="w-1.5 h-1.5 rounded-full bg-amber-500/60" />
      ...
      {/* Left panel */}
      <div ref={leftPanelRef} className="absolute left-0 top-0 bottom-0 w-[22%] flex flex-col items-center justify-center gap-8 pointer-events-none px-4 z-[3]">
        ...
        <div className="border border-amber-400/30 rounded-sm p-4 text-center bg-white/5 backdrop-blur-md w-full max-w-[140px]">
          ...
          <p className="font-cinzel text-[10px] tracking-widest uppercase text-amber-100/60 leading-relaxed">Nelumbo<br />Nucifera</p>
          <div className="w-8 h-px bg-amber-400/30 mx-auto mt-2" />
          <p className="font-cinzel text-[9px] text-amber-100/40 mt-2 leading-relaxed">Sacred flower of<br />the Buddha</p>
        </div>
        <div className="flex flex-col items-center gap-2">
          {[1, 0.5, 0.3].map((op, i) => <div key={i} className="w-1 h-1 rounded-full bg-amber-400" style={{ opacity: op }} />)}
        </div>
      ...
      {/* Right panel */}
      <div ref={rightPanelRef} className="absolute right-0 top-0 bottom-0 w-[22%] flex flex-col items-center justify-center gap-8 pointer-events-none px-4 z-[3]">
        <div className="flex flex-col items-center gap-1">
        ...
        <div className="border border-rose-300/25 rounded-sm p-4 text-center bg-white/5 backdrop-blur-md w-full max-w-[140px]">
          ...
          <p className="font-cinzel text-[10px] tracking-widest uppercase text-rose-100/60 leading-relaxed">Dhamma<br />Chakra</p>
          <div className="w-8 h-px bg-rose-300/30 mx-auto mt-2" />
          <p className="font-cinzel text-[9px] text-rose-100/40 mt-2 leading-relaxed">Wheel of the<br />Dharma</p>
        </div>
        <div className="flex flex-col items-center gap-2">
          {[0.3, 0.5, 1].map((op, i) => <div key={i} className="w-1 h-1 rounded-full bg-rose-300" style={{ opacity: op }} />)}
        </div>
        <div className="text-center">
      ...
      <div className="absolute inset-0 flex items-center justify-center z-[3] pointer-events-none">
        ...
        {/* Glows */}
        <div ref={glowRef} className="absolute w-96 h-96 rounded-full pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(212,175,55,0.45) 0%, rgba(212,175,55,0.12) 55%, transparent 78%)", filter: "blur(36px)" }}
        />
        <div ref={glowRoseRef} className="absolute w-[480px] h-[480px] rounded-full pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(255,182,193,0.4) 0%, rgba(255,192,203,0.1) 55%, transparent 78%)", filter: "blur(40px)" }}
        />

        {/* Mandala */}
        <svg ref={mandalaRef} className="absolute pointer-events-none" width="560" height="560" viewBox="-280 -280 560 560" style={{ transformOrigin: "50% 50%" }}>
          ...
          {MANDALA_TICKS.map((t, i) => (
            <line key={i} x1={t.x1 * 1.31} y1={t.y1 * 1.31} x2={t.x2 * 1.31} y2={t.y2 * 1.31}
          ...
          {Array.from({ length: 8 }, (_, i) => {
            ...
            return <circle key={i} cx={cx} cy={cy} r={i % 2 === 0 ? 3 : 2} fill="rgba(212,175,55,0.35)" />;
          })}
        </svg>

        {/* Rings */}
        <div ref={ring2Ref} className="absolute w-[580px] h-[580px] rounded-full border border-rose-300/15 pointer-events-none" />
        <div ref={ring1Ref} className="absolute w-[480px] h-[480px] rounded-full border border-amber-400/20 pointer-events-none" />

        {/* Lotus */}
        <div ref={lotusRef} className="relative flex items-center justify-center" style={{ width: 460, height: 460, isolation: "isolate" }}>
          {PETALS.map((p) => (
            <svg key={p.gradId} data-layer={p.li} className="lotus-petal-svg absolute" viewBox="0 0 60 100"
              style={{ width: p.w, height: p.h, left: p.left, top: p.top, transform: `rotate(${p.angle}deg)`, transformOrigin: "50% 100%", zIndex: p.zIndex, filter: p.filter }}
            >
              <defs>
              ...
              <path d={p.path} fill={`url(#${p.gradId})`} stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
              <line x1="30" y1="15" x2="30" y2="88" stroke="rgba(255,255,255,0.12)" strokeWidth="0.6" />
            </svg>
          ))}

          {/* Stamen */}
          <div ref={centerRef} className="absolute z-20" style={{ width: 72, height: 72, top: "50%", left: "50%", transform: "translate(-50%, -50%)" }}>
            {STAMEN_DOTS.map((dot, i) => (
              <div key={`o${i}`} className="absolute rounded-full"
                style={{ width: 5, height: 5, background: "radial-gradient(circle at 40% 30%, #fff9c4, #f9a825)", top: "50%", left: "50%", transform: dot.transform, boxShadow: "0 0 3px rgba(249,168,37,0.9)" }}
              />
            ))}
            {STAMEN_INNER.map((dot, i) => (
              <div key={`in${i}`} className="absolute rounded-full"
                style={{ width: 4, height: 4, background: "radial-gradient(circle at 40% 30%, #fff9c4, #f57f17)", top: "50%", left: "50%", transform: dot.transform, boxShadow: "0 0 3px rgba(245,127,23,0.9)" }}
              />
            ))}
            <div className="absolute rounded-full"
              style={{ width: 28, height: 28, top: "50%", left: "50%", transform: "translate(-50%, -50%)", background: "radial-gradient(circle at 38% 32%, #fff9c4 0%, #f9c74f 40%, #e65100 100%)", boxShadow: "0 0 0 3px rgba(249,199,79,0.4), 0 0 20px 8px rgba(212,175,55,0.5)" }}
            />
          </div>
        </div>
      </div>
      ...
      {/* Quote + attributes */}
      <div className="absolute bottom-20 left-0 right-0 flex flex-col items-center gap-4 z-[3] pointer-events-none">
        ...
        <div ref={attributesRef} className="flex items-center gap-10">
        ...
        <div ref={subtitleRef} className="text-center">
          ...
          <p className="font-cinzel text-[10px] text-amber-100/40 tracking-[0.3em] uppercase">
      ...
      {/* Bottom bar */}
      <div ref={bottomLineRef} className="absolute bottom-8 left-0 right-0 flex items-center justify-center gap-4 pointer-events-none z-[3]" style={{ transformOrigin: "50% 50%" }}>
        ...
        <p className="font-cinzel text-[9px] tracking-[0.4em] uppercase text-amber-100/40">Aba Ceylon Tours &amp; Travels</p>
        <span className="font-cinzel text-amber-400/50 text-xs">✦</span>
