...
export default function DestinationPanel({ destination, onClose }: DestinationPanelProps) {
  // Close on Escape key
  useEffect(() => {
    ...
    <>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 w-full max-w-2xl max-h-[85vh] overflow-hidden">
        <div className="bg-white rounded-2xl shadow-2xl mx-4 overflow-hidden animate-slideUp">
          {/* Header */}
          <div className={`${categoryColors[destination.category]} p-6 text-white relative`}>
            <button
              onClick={onClose}
              ...
              <ul className="space-y-2">
                {destination.highlights.map((highlight, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <span className={`${categoryColors[destination.category]} w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5`}>
