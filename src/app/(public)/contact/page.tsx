    ...
    <main className="relative min-h-screen overflow-hidden bg-[#08111d] text-white">
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-20 pt-32 sm:px-6 lg:px-8 lg:pt-36">
        <div className="mb-12 max-w-3xl">
          <p className="font-cinzel text-xs uppercase tracking-[0.35em] text-amber-300/75">{t("contact.eyebrow")}</p>
          <h1 className="mt-4 font-cinzel text-4xl leading-tight text-white sm:text-5xl lg:text-6xl">{t("contact.title")}</h1>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.08fr_0.92fr]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="mb-2 block text-sm text-white/70" style={SWITZER}>{t("contact.name")}</span>
                  <input type="text" name="name" value={form.name} onChange={handleChange} placeholder={t("contact.name")} className={INPUT_CLS} style={SWITZER} />
                </label>
                <label className="block">
                  <span className="mb-2 block text-sm text-white/70" style={SWITZER}>{t("contact.email")}</span>
                  <input type="email" name="email" value={form.email} onChange={handleChange} placeholder={t("contact.email")} className={INPUT_CLS} style={SWITZER} />
                </label>
              </div>

              <label className="block">
                <span className="mb-2 block text-sm text-white/70" style={SWITZER}>{t("contact.phone")}</span>
                <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder={t("contact.phone")} className={INPUT_CLS} style={SWITZER} />
              </label>

              <label className="block">
                <span className="mb-2 block text-sm text-white/70" style={SWITZER}>{t("contact.message")}</span>
                <textarea
              ...
              <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center">
                <Button type="submit" variant="secondary" size="lg">{t("contact.submit")}</Button>
                <p className="text-sm text-white/55" style={SWITZER}>{t("contact.submitHint")}</p>
              </div>
            </form>
          </div>
          ...
          <div className="space-y-8">
            <Card variant="dark" className="p-6 sm:p-8 bg-[#111c2f]/80">
              <h2 className="font-cinzel text-3xl text-white sm:text-4xl">{t("contact.callUs")}</h2>
              <p className="mt-3 text-base text-white/65" style={SWITZER}>{t("contact.callDescription")}</p>
              ...
              <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <Link
                  ...
                  style={SWITZER}
                >
                  <MessageCircle className="h-4 w-4" /> {t("contact.whatsappUs")}
                </Link>
              </div>

              <div className="mt-8 space-y-4 text-white/72">
                {[
                  { icon: <Phone className="mt-1 h-5 w-5 text-amber-300/80" />, label: t("contact.phoneLabel"), value: formattedPhone, href: undefined },
                  { icon: <Mail className="mt-1 h-5 w-5 text-amber-300/80" />, label: t("contact.emailLabel"), value: ADMIN_EMAIL, href: `mailto:${ADMIN_EMAIL}` },
                  { icon: <MapPin className="mt-1 h-5 w-5 text-amber-300/80" />, label: t("contact.basedIn"), value: "Colombo, Sri Lanka", href: undefined },
            ...
            <Card variant="glass" className="p-6 sm:p-8">
              <h2 className="font-cinzel text-3xl text-white sm:text-4xl">{t("contact.openingHours")}</h2>
              <p className="mt-3 text-base text-white/65" style={SWITZER}>
                {t("contact.openingDescription")}
              </p>
