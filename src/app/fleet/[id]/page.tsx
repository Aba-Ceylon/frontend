...
export default function VehiclePage() {
  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="font-cinzel text-2xl text-neutral-700">{t("fleet.notFound")}</p>
      </div>
    );
  }
  ...
  if (!vehicle) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="font-cinzel text-neutral-500">{t("fleet.loadingVehicle")}</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white">
      <div className="relative h-96 w-full bg-gray-100">
        ...
        <div className="absolute inset-0 flex items-end p-8">
          <div>
            <Badge variant="amber" className="mb-3">{t("fleet.premiumFleet")}</Badge>
        ...
        <div className="max-w-4xl mx-auto py-12">
          <div className="flex flex-col lg:flex-row gap-8">
            <Card variant="white" className="lg:w-2/3 p-6">
              <h2 className="font-cinzel text-2xl text-[#0b2545] mb-4">{t("fleet.overview")}</h2>
              <p className="text-neutral-700 mb-4">{t("fleet.models")}: {vehicle.models.join(", ")}</p>

              <div className="flex items-center gap-4 mb-6 flex-wrap">
                <Badge variant="light" className="bg-slate-100 text-slate-700 border-slate-200 gap-1.5">
                  <Users size={13} />
                  {t("fleet.passengers", { count: vehicle.passengerCapacity })}
                </Badge>
                <Badge variant="light" className="bg-slate-100 text-slate-700 border-slate-200 gap-1.5">
                  <Briefcase size={13} />
                  {t("fleet.luggage", { count: vehicle.luggageCapacity })}
                </Badge>
              </div>

              <h3 className="font-cinzel text-lg text-[#0b2545] mb-3">{t("fleet.features")}</h3>
              ...
              <h3 className="font-cinzel text-lg text-[#0b2545] mb-2">{t("fleet.aboutVehicle")}</h3>
              <p className="text-neutral-700 leading-7">{vehicle.shortDescription}</p>
            </Card>

            <aside className="lg:w-1/3">
              <Card variant="white" className="p-6">
                <h4 className="font-cinzel text-lg text-[#0b2545] mb-4">{t("fleet.requestVehicle")}</h4>
                <p className="text-neutral-700 mb-4 text-sm leading-6">
                  {t("fleet.requestDescription")}
                </p>
